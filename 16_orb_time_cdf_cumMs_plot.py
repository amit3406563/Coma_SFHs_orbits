...
def plot_all(ext,ssp):
    ...
    pdf = PdfPages(out_dir+ext+'_'+ssp+'_'+'cdf_inf_cum_Ms.pdf')
    for i_frac, name in zip(int_frac,gmp_names):
        if name == '3329':
            continue
        else:
            fig, ax = plt.subplots(figsize=(10,7))
            f50, g50, f16, f84, g16, g84, xerr, yerr, be, m_cdf, time_cdf, be_neg = err_plots(name,'inf',i_frac,inf_df,peri_df,sfr_df)
    ...
    pdf = PdfPages(out_dir+ext+'_'+ssp+'_'+'cdf_peri_cum_Ms.pdf')
    for i_frac, name in zip(int_frac,gmp_names):
        if name == '3329':
            continue
        else:
            fig, ax = plt.subplots(figsize=(10,7))
            f50, g50, f16, f84, g16, g84, xerr, yerr, be, m_cdf, time_cdf, be_neg = err_plots(name,'peri',i_frac,inf_df,peri_df,sfr_df)
            ax.plot(be/10**9,1-m_cdf,color='tab:red',linestyle='-',
                    label=r'Cum. stellar mass ($M_\star$)')
...
###############################################################################
## Plotting CDFs of Infall and Pericenter and Cumulative Stellar mass vs time ##
###############################################################################
