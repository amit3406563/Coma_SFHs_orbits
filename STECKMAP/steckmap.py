...
#    plt.show()
    ...
    if (str(datafiles[i]).find('spectra') != -1):
        ...
        yaxis3 = yaxis[2*n:3*n]
        yaxis4 = yaxis[3*n:4*n]
        out_dat = pd.DataFrame({'Wavelength (A)':xaxis, 'Specta':yaxis1,
                                'Best Fitting Model':yaxis2,
                                'Non-parm Extinction Curve': yaxis3,
                                'Non-param Adjusted Continuum': yaxis4})
        out_dat.to_csv('./out_files/'+filenames[i]+'.dat',index=None,sep=' ')
