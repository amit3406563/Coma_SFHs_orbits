...
int_frac = np.array(pd.read_csv('int_frac.csv')['int_frac'])
...
inf_df = pd.read_csv('inf_time.csv')
peri_df = pd.read_csv('peri_time.csv')

# plotting indiviually for sanity checks - infall time
pdf = PdfPages('cdf_inf_cum_Ms.pdf')
...
pdf = PdfPages('cdf_peri_cum_Ms.pdf')
for i_frac, name in zip(int_frac,gmp_names):
    if name == '3329':
        continue
    else:
